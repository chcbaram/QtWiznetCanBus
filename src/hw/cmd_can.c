...
{
  return p_cmd->p_driver->available(p_cmd->p_driver->p_arg);
}
...
bool cmdCanReceivePacket(cmd_can_t *p_cmd)
{
  ...
  while(ret != true && p_driver->available(p_driver->p_arg) > 0)
  {
    rx_data = p_driver->read(p_driver->p_arg);
...
bool cmdCanSend(cmd_can_t *p_cmd, CmdCanType_t type, CmdCanCmd_t cmd, uint8_t *p_data, uint32_t length)
{
  ...
  wr_len = p_driver->write(p_driver->p_arg, p_cmd->tx_packet.buffer, index);
